...
// New creates a new Feechain object (including the
// initialisation of the common Feechain object)
func New(
...
func (fch *Feechain) IsLeader() bool {
	return fch.NodeAPI.IsCurrentlyLeader()
}

// GetNodeMetadata ..
func (fch *Feechain) GetNodeMetadata() commonRPC.NodeMetadata {
	header := fch.CurrentBlock().Header()
	...
	c := commonRPC.C{}
	c.TotalKnownPeers, c.Connected, c.NotConnected = fch.NodeAPI.PeerConnectivity()

	syncPeers := fch.NodeAPI.SyncPeers()
	consensusInternal := fch.NodeAPI.GetConsensusInternal()

	return commonRPC.NodeMetadata{
		BLSPublicKey:    blsKeys,
		Version:         nodeconfig.GetVersion(),
		NetworkType:     string(cfg.GetNetworkType()),
		ChainConfig:     *fch.ChainConfig(),
		IsLeader:        fch.IsLeader(),
		ShardID:         fch.ShardID,
		...
		IsBackup:        fch.NodeAPI.IsBackup(),
		NodeBootTime:    fch.NodeAPI.GetNodeBootTime(),
...
func (fch *Feechain) GetEVM(ctx context.Context, msg core.Message, state *state.DB, header *block.Header) (*vm.EVM, error) {
	...
	vmCtx := core.NewEVMContext(msg, header, fch.BlockChain, nil)
	return vm.NewEVM(vmCtx, state, fch.BlockChain.Config(), *fch.BlockChain.GetVMConfig()), nil
}
...
func (fch *Feechain) BloomStatus() (uint64, uint64) {
	sections, _, _ := fch.BloomIndexer.Sections()
