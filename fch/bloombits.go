...
func (fch *Feechain) startBloomHandlers(sectionSize uint64) {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-fch.ShutdownChan:
					return

				case request := <-fch.BloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(fch.chainDb, (section+1)*sectionSize-1)
						if compVector, err := rawdb.ReadBloomBits(fch.chainDb, task.Bit, section, head); err == nil {
