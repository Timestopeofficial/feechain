...
func (fch *Feechain) TraceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	database := state.NewDatabaseWithCache(fch.ChainDb(), 16)

	if origin > 0 {
		start = fch.BlockChain.GetBlock(start.ParentHash(), origin-1)
		if start == nil {
	...
	if err != nil {
		...
		for i := uint64(0); i < reexec; i++ {
			start = fch.BlockChain.GetBlock(start.ParentHash(), start.NumberU64()-1)
			if start == nil {
				break
			}
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range tasks {
				fchSigner := types.MakeSigner(fch.BlockChain.Config(), task.block.Number())
				ethSigner := types.NewEIP155Signer(fch.BlockChain.Config().EthCompatibleChainID)
				...
				for i, tx := range task.block.Transactions() {
					...
					vmCtx := core.NewEVMContext(msg, task.block.Header(), fch.BlockChain, nil)

					res, err := fch.TraceTx(ctx, msg, vmCtx, task.statedb, config)
					if err != nil {
						task.results[i] = &TxTraceResult{Error: err.Error()}
						utils.Logger().Warn().Msg("Tracing failed")
						break
					}
					// EIP 158/161 (Spurious Dragon) does not apply to Feechain
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			block := fch.BlockChain.GetBlockByNumber(number)
			if block == nil {
			...
			_, _, _, _, _, _, _, err := fch.BlockChain.Processor().Process(block, statedb, vm.Config{}, false)
			if err != nil {
				failed = err
				break
			}
...
func (fch *Feechain) traceBlockNoThread(ctx context.Context, block *types.Block, config *TraceConfig) ([]*TxTraceResult, error) {
	// Create the parent state database
	if err := fch.BlockChain.Engine().VerifyHeader(fch.BlockChain, block.Header(), true); err != nil {
		return nil, err
	}
	parent := fch.BlockChain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	statedb, err := fch.ComputeStateDB(parent, reexec)
	if err != nil {
		return nil, err
	}
	// Execute all the transaction contained within the block concurrently
	var (
		fchSigner = types.MakeSigner(fch.BlockChain.Config(), block.Number())
		ethSigner = types.NewEIP155Signer(fch.BlockChain.Config().EthCompatibleChainID)
		txs       = block.Transactions()
...
traceLoop:
	for i, tx := range txs {
		...
		vmctx := core.NewEVMContext(msg, block.Header(), fch.BlockChain, nil)
		res, err := fch.TraceTx(ctx, msg, vmctx, statedb, config)
		if err != nil {
...
func (fch *Feechain) TraceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*TxTraceResult, error) {
	...
	if *config.Tracer == "ParityBlockTracer" {
		return fch.traceBlockNoThread(ctx, block, config)
	}
	// Create the parent state database
	if err := fch.BlockChain.Engine().VerifyHeader(fch.BlockChain, block.Header(), true); err != nil {
		return nil, err
	...
	for i, tx := range txs {
		...
		vmenv := vm.NewEVM(vmctx, statedb, fch.BlockChain.Config(), vm.Config{})
...
func (fch *Feechain) standardTraceBlockToFile(ctx context.Context, block *types.Block, config *StdTraceConfig) ([]string, error) {
	...
	for i, tx := range block.Transactions() {
		...
		vmenv := vm.NewEVM(vmctx, statedb, fch.BlockChain.Config(), vmConf)
...
func (fch *Feechain) ComputeStateDB(block *types.Block, reexec uint64) (*state.DB, error) {
	...
	statedb, err := fch.BlockChain.StateAt(block.Root())
	if err == nil {
		return statedb, nil
	}
	// Otherwise try to reexec blocks until we find a state or reach our limit
	origin := block.NumberU64()
	database := state.NewDatabaseWithCache(fch.BlockChain.ChainDb(), 16)

	for i := uint64(0); i < reexec; i++ {
		block = fch.BlockChain.GetBlock(block.ParentHash(), block.NumberU64()-1)
		if block == nil {
			break
		}
	...
	for block.NumberU64() < origin {
		...
		if block = fch.BlockChain.GetBlockByNumber(block.NumberU64() + 1); block == nil {
...
func (fch *Feechain) TraceTx(ctx context.Context, message core.Message, vmctx vm.Context, statedb *state.DB, config *TraceConfig) (interface{}, error) {
	...
	vmenv := vm.NewEVM(vmctx, statedb, fch.BlockChain.Config(), vm.Config{Debug: true, Tracer: tracer})
...
func (fch *Feechain) ComputeTxEnv(block *types.Block, txIndex int, reexec uint64) (core.Message, vm.Context, *state.DB, error) {
	...
	for idx, tx := range block.Transactions() {
		...
		msg, _ := tx.AsMessage(signer)
		context := core.NewEVMContext(msg, block.Header(), fch.BlockChain, nil)
		if idx == txIndex {
			return msg, context, statedb, nil
		}
		// Not yet the searched for transaction, execute on top of the current state
		vmenv := vm.NewEVM(context, statedb, fch.BlockChain.Config(), vm.Config{})
