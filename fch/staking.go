...
func (fch *Feechain) readAndUpdateRawStakes(
	epoch *big.Int,
	...
	for i := range comm.Slots {
		...
		if !ok {
			snapshot, err := fch.BlockChain.ReadValidatorSnapshotAtEpoch(epoch, slotAddr)
			if err != nil {
				continue
			}
...
func (fch *Feechain) getSuperCommittees() (*quorum.Transition, error) {
	nowE := fch.BlockChain.CurrentHeader().Epoch()

	if fch.BlockChain.CurrentHeader().IsLastBlockInEpoch() {
	...
	nowCommittee, err = fch.BlockChain.ReadShardState(nowE)
	if err != nil {
		return nil, err
	}
	prevCommittee, err = fch.BlockChain.ReadShardState(thenE)
	if err != nil {
		return nil, err
	}
	...
	for _, comm := range prevCommittee.Shards {
		...
		if fch.BlockChain.Config().IsStaking(prevCommittee.Epoch) {
			if _, err := decider.SetVoters(&comm, prevCommittee.Epoch); err != nil {
				return nil, err
			}
		}
		rawStakes = fch.readAndUpdateRawStakes(thenE, decider, comm, rawStakes, validatorSpreads)
	...
	for _, comm := range nowCommittee.Shards {
		...
		if _, err := decider.SetVoters(&comm, nowCommittee.Epoch); err != nil {
			return nil, errors.Wrapf(
				...
				fch.BlockChain.Config().StakingEpoch,
				fch.BlockChain.CurrentHeader().Epoch(),
			)
		}
		rawStakes = fch.readAndUpdateRawStakes(nowE, decider, comm, rawStakes, validatorSpreads)
...
func (fch *Feechain) IsStakingEpoch(epoch *big.Int) bool {
	return fch.BlockChain.Config().IsStaking(epoch)
}

// IsPreStakingEpoch ...
// func (fch *Feechain) IsPreStakingEpoch(epoch *big.Int) bool {
// 	return fch.BlockChain.Config().IsPreStaking(epoch)
// }

// IsNoEarlyUnlockEpoch ...
// func (fch *Feechain) IsNoEarlyUnlockEpoch(epoch *big.Int) bool {
// 	return fch.BlockChain.Config().IsNoEarlyUnlock(epoch)
// }
...
func (fch *Feechain) IsCommitteeSelectionBlock(header *block.Header) bool {
	return chain.IsCommitteeSelectionBlock(fch.BlockChain, header)
}

// GetDelegationLockingPeriodInEpoch ...
func (fch *Feechain) GetDelegationLockingPeriodInEpoch(epoch *big.Int) int {
	return chain.GetLockPeriodInEpoch(fch.BlockChain, epoch)
}
...
func (fch *Feechain) SendStakingTx(ctx context.Context, signedStakingTx *staking.StakingTransaction) error {
	stx, _, _, _ := rawdb.ReadStakingTransaction(fch.chainDb, signedStakingTx.Hash())
	if stx == nil {
		return fch.NodeAPI.AddPendingStakingTransaction(signedStakingTx)
	}
...
func (fch *Feechain) GetStakingTransactionsHistory(address, txType, order string) ([]common.Hash, error) {
	return fch.NodeAPI.GetStakingTransactionsHistory(address, txType, order)
}
...
func (fch *Feechain) GetStakingTransactionsCount(address, txType string) (uint64, error) {
	return fch.NodeAPI.GetStakingTransactionsCount(address, txType)
}
...
func (fch *Feechain) GetSuperCommittees() (*quorum.Transition, error) {
	...
	res, err := fch.SingleFlightRequest(
		key, func() (interface{}, error) {
			thenE := new(big.Int).Sub(nowE, common.Big1)
			thenKey := fmt.Sprintf("sc-%s", thenE.String())
			fch.group.Forget(thenKey)
			return fch.getSuperCommittees()
		})
	if err != nil {
		return nil, err
	}
...
func (fch *Feechain) GetValidators(epoch *big.Int) (*shard.Committee, error) {
	state, err := fch.BlockChain.ReadShardState(epoch)
	if err != nil {
		return nil, err
	}
	for _, cmt := range state.Shards {
		if cmt.ShardID == fch.ShardID {
			return &cmt, nil
		}
	}
	return nil, nil
...
func (fch *Feechain) GetValidatorSelfDelegation(addr common.Address) *big.Int {
	wrapper, err := fch.BlockChain.ReadValidatorInformation(addr)
...
func (fch *Feechain) GetElectedValidatorAddresses() []common.Address {
	list, _ := fch.BlockChain.ReadShardState(fch.BlockChain.CurrentBlock().Epoch())
...
func (fch *Feechain) GetAllValidatorAddresses() []common.Address {
	return fch.BlockChain.ValidatorCandidates()
}
...
func (fch *Feechain) getEpochSigning(epoch *big.Int, addr common.Address) (staking.EpochSigningEntry, error) {
	...
	snapshot, err := fch.BlockChain.ReadValidatorSnapshotAtEpoch(epoch, addr)
	if err != nil {
		return entry, err
	}
	...
	prevEpochSnap, err := fch.BlockChain.ReadValidatorSnapshotAtEpoch(prevEpoch, addr)
	if err == nil {
...
func (fch *Feechain) GetValidatorInformation(
	...
	lastBlockOfEpoch := shard.Schedule.EpochLastBlock(fch.BeaconChain.CurrentBlock().Header().Epoch().Uint64())

	computed.BlocksLeftInEpoch = lastBlockOfEpoch - fch.BeaconChain.CurrentBlock().Header().Number().Uint64()
	...
	for i := now.Int64(); i > epochFrom.Int64(); i-- {
		epoch := big.NewInt(i)
		entry, err := fch.getEpochSigning(epoch, addr)
		if err != nil {
			break
		}
...
func (fch *Feechain) GetMedianRawStakeSnapshot() (
	*committee.CompletedEPoSRound, error,
	...
	blockNum := fch.CurrentBlock().NumberU64()
	key := fmt.Sprintf("median-%d", blockNum)

	// delete cache for previous block
	prevKey := fmt.Sprintf("median-%d", blockNum-1)
	fch.group.Forget(prevKey)

	res, err := fch.SingleFlightRequest(
		key,
		func() (interface{}, error) {
			// Compute for next epoch
			epoch := big.NewInt(0).Add(fch.CurrentBlock().Epoch(), big.NewInt(1))
			return committee.NewEPoSRound(epoch, fch.BlockChain, true)
			// return committee.NewEPoSRound(epoch, fch.BlockChain, fch.BlockChain.Config().IsEPoSBound35(epoch))
		},
	)
	if err != nil {
		return nil, err
...
func (fch *Feechain) GetDelegationsByValidator(validator common.Address) []*staking.Delegation {
	wrapper, err := fch.BlockChain.ReadValidatorInformation(validator)
	if err != nil || wrapper == nil {
		return nil
	}
...
func (fch *Feechain) GetDelegationsByValidatorAtBlock(
	...
	wrapper, err := fch.BlockChain.ReadValidatorInformationAtRoot(validator, block.Root())
	if err != nil || wrapper == nil {
		return nil
	}
	delegations := []*staking.Delegation{}
...
func (fch *Feechain) GetDelegationsByDelegator(
	...
	block := fch.BlockChain.CurrentBlock()
	return fch.GetDelegationsByDelegatorByBlock(delegator, block)
}
...
func (fch *Feechain) GetDelegationsByDelegatorByBlock(
	...
	delegations := []*staking.Delegation{}
	delegationIndexes, err := fch.BlockChain.
	...
	for i := range delegationIndexes {
		wrapper, err := fch.BlockChain.ReadValidatorInformationAtRoot(
...
func (fch *Feechain) GetUndelegationPayouts(
	...
	// if !fch.IsPreStakingEpoch(epoch) {
	// 	return nil, fmt.Errorf("not pre-staking epoch or later")
	// }

	payouts, ok := fch.undelegationPayoutsCache.Get(epoch.Uint64())
	if ok {
	...
	undelegationPayoutBlock, err := fch.BlockByNumber(ctx, rpc.BlockNumber(blockNumber))
	...
	lockingPeriod := fch.GetDelegationLockingPeriodInEpoch(undelegationPayoutBlock.Epoch())
	for _, validator := range fch.GetAllValidatorAddresses() {
		wrapper, err := fch.BlockChain.ReadValidatorInformationAtRoot(validator, undelegationPayoutBlock.Root())
		if err != nil || wrapper == nil {
			continue // Not a validator at this epoch or unable to fetch validator info because of pruned state.
		}
		// noEarlyUnlock := fch.IsNoEarlyUnlockEpoch(epoch)
	...
	fch.undelegationPayoutsCache.Add(epoch.Uint64(), undelegationPayouts)
	return undelegationPayouts, nil
}

// GetTotalStakingSnapshot ..
func (fch *Feechain) GetTotalStakingSnapshot() *big.Int {
	if stake := fch.totalStakeCache.pop(fch.CurrentBlock().NumberU64()); stake != nil {
		return stake
	}
	currHeight := fch.CurrentBlock().NumberU64()
	candidates := fch.BlockChain.ValidatorCandidates()
	if len(candidates) == 0 {
		stake := big.NewInt(0)
		fch.totalStakeCache.push(currHeight, stake)
		return stake
	}
	stakes := big.NewInt(0)
	for i := range candidates {
		snapshot, _ := fch.BlockChain.ReadValidatorSnapshot(candidates[i])
		validator, _ := fch.BlockChain.ReadValidatorInformation(candidates[i])
	...
	fch.totalStakeCache.push(currHeight, stakes)
	return stakes
}

// GetCurrentStakingErrorSink ..
func (fch *Feechain) GetCurrentStakingErrorSink() types.TransactionErrorReports {
	return fch.NodeAPI.ReportStakingErrorSink()
}
