...
DOWNLOAD_URL=https://github.com/Timestopeofficial/feechain/releases/download
...
get_version() {
    if [[ -n ${HARMONY_RELEASE:-} ]]; then
        ...
        harmony_rel=$(curl -f -s https://harmony.one/releases/latest) || (color "31" "Get latest version failed. You may manually download the binaries from Github release page. https://github.com/Timestopeofficial/feechain/releases/" && exit 1)
