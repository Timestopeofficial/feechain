...
FOR /F "tokens=* USEBACKQ" %%F IN (`docker ps -q -a -f "name=^feechain-%account_id%-%port_base%$"`) DO (
	SET running_container=%%F
)

IF NOT "%running_container%"=="" (
    ECHO Stop node for account id: %account_id% port %port_base%
	docker rm -v -f feechain-%account_id%-%port_base% >NUL
)
...
docker run -it -d ^
	--name feechain-%account_id%-%port_base% ^
	-p %port_base%:%port_base% -p %port_rest%:%port_rest% -p %port_rpc%:%port_rpc% ^
	-e NODE_PORT=%port_base% ^
	-e NODE_ACCOUNT_ID=%account_id% ^
	--mount type=volume,source=db-%account_id%-%port_base%,destination=/feechain/db ^
	--mount type=volume,source=log-%account_id%-%port_base%,destination=/feechain/log ^
	%DOCKER_IMAGE% >NUL

echo.
echo ======================================
echo Node for account %account_id% (port %port_base%) is running in container 'feechain-%account_id%-%port_base%'"
echo.
echo To check console log, please run `docker logs -f feechain-%account_id%-%port_base%`"
