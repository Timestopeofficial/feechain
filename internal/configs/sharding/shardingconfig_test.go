...
func TestGetShardingStructure(t *testing.T) {
	shardID := 0
	numShard := 2
	res := genShardingStructure(numShard, shardID, "http://s%d.t.timestope.net:17500", "ws://s%d.t.timestope.net:17800")
	...
	for i := 0; i < numShard; i++ {
		...
		if res[i]["http"].(string) != fmt.Sprintf("http://s%d.t.timestope.net:17500", i) {
			t.Error("Error when generating sharding structure")
		}
		if res[i]["ws"].(string) != fmt.Sprintf("ws://s%d.t.timestope.net:17800", i) {
			t.Error("Error when generating sharding structure")
		}
	}
}
